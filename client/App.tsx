...
function EnvelopeIcon({ className = '' }: { className?: string }) {
  return <svg className={className} width="22" height="20" viewBox="0 0 22 20" fill="none" aria-hidden="true"><path d="M2 4h18v13H2zM2 4l9 7 9-7" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" /></svg>;
}
function Arrow({ down = false }: { down?: boolean }) {
  return <svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true" style={down ? { transform: 'rotate(90deg)' } : undefined}><path d="M3 10h13m-5-5 5 5-5 5" stroke="currentColor" strokeWidth="1.5" /></svg>;
}
...
function Receipt({ receipt, openBin }: { receipt: SavedReceipt; openBin: (category: Category, id?: string) => void }) {
  ...
  return <div className={`receipt ${done ? 'receipt-done' : ''}`} aria-live="polite">
    <span className={`receipt-icon ${!terminal.has(status) ? 'receipt-pending' : ''}`}>{done ? '✓' : discarded ? '↳' : <EnvelopeIcon />}</span>
...
function HistoryPanel({ category, highlight, room, onSelect, onClose }: { category: Category; highlight: string | null; room: RoomSnapshot | null; onSelect: (category: Category, id?: string) => void; onClose: () => void }) {
  ...
  return <div className="panel-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="history-panel" ref={panel} role="dialog" aria-modal="true" aria-labelledby="bin-title">
      <div className="panel-top"><span className="eyebrow">THE MESSAGE ARCHIVE</span><button className="icon-button" ref={closeButton} onClick={onClose} aria-label="Close message archive">×</button></div>
      <div className="panel-heading"><span className="big-bin" style={{ '--bin-color': meta.color } as React.CSSProperties}><EnvelopeIcon /></span><div><h2 id="bin-title">{meta.label}</h2><p>{meta.description}</p></div></div>
      ...
      {newMessages && <button className="new-messages" onClick={() => { scroll.current?.scrollTo({ top: 0, behavior: 'smooth' }); void refresh().catch(error => setError(error.message)); }}>New messages have arrived ↑</button>}
      <div className="message-list" ref={scroll}>
        ...
        {messages.map(message => <article key={message.id} className={`message-card ${message.id === highlight ? 'highlighted' : ''}`}>
          ...
          <p className="message-body">{message.text}</p><div className="jev-reaction"><span className="mini-face">j</span><span>{message.reaction}</span></div>
        </article>)}
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="site-header"><a href="/" className="brand" aria-label="Jev’s Mailroom home"><span className="brand-mark"><EnvelopeIcon /></span><span>jev’s mailroom<span className="brand-period">.</span></span></a><div className="header-right"><span className="room-live"><span className={`status-dot ${connected ? '' : 'offline'}`} />{connected ? 'THE MAILROOM IS OPEN' : 'CONNECTING TO THE MAILROOM'}</span><a href="#how-it-works" className="about-link">What is this? <span>↗</span></a></div></header>
    <main>
      <section className="intro"><div><div className="eyebrow intro-eyebrow"><span className="little-star">✳</span> SMALL NOTES. A SHARED LITTLE WORLD.</div><h1>Every thought has <em>a place.</em></h1></div><p>Leave a note. Watch Jev find it a home.<br />A tiny mailroom, open to everyone.</p></section>
      <div className="main-layout">
        <section className="mailroom" aria-label="Shared live mailroom">
          <div className="room-toolbar"><span><span className="status-dot" /> LIVE FROM THE MAILROOM</span><span className="visitors"><svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden="true"><circle cx="8" cy="5" r="2.5" stroke="currentColor" /><path d="M3 14v-2a5 5 0 0 1 10 0v2" stroke="currentColor" /></svg> {room ? `${room.online} here now` : 'Opening the door…'}</span></div>
          ...
          {active && <div className="reaction-bubble" aria-live="polite"><span className="mini-face">j</span><span>“{active.reaction}”</span></div>}
          ...
          <div className="bin-grid">{CATEGORIES.map((category, i) => <button key={category} className="bin-card" onClick={() => openBin(category)} style={{ '--bin-color': BIN_META[category].color } as React.CSSProperties} aria-label={`Browse ${BIN_META[category].label}, ${countLabel(room?.counts[category] || 0)}`}>
            <div className="bin-card-top"><span className="bin-number">0{i + 1}</span><span className="small-bin"><span /></span><Arrow /></div><span className="bin-label">{BIN_META[category].label}</span><span className="bin-count">{countLabel(room?.counts[category] || 0)}</span>
          </button>)}</div>
        </section>
        <aside className="compose-column">
          <section className="composer"><div className="composer-stamp"><EnvelopeIcon /><span>TO: JEV<br />WITH THOUGHT</span></div><span className="eyebrow">GOT SOMETHING ON YOUR MIND?</span><h2>Send a little note.</h2><p>A kind word, a bright idea, a small complaint. Jev knows just the place.</p>
            <form onSubmit={send}><label htmlFor="message" className="sr-only">Your message to Jev</label><div className="textarea-wrap"><textarea id="message" ref={textarea} value={text} onChange={event => setText(event.target.value)} placeholder={"Dear Jev,\nI’ve been thinking…"} maxLength={280} rows={5} required disabled={sending} aria-describedby="public-note character-count" /><span id="character-count" className={text.length > 260 ? 'character-count near-limit' : 'character-count'}>{text.length}<span> / 280</span></span></div>
              <button className="send-button" type="submit" disabled={!text.trim() || sending}>{sending ? 'Handing it to Jev…' : 'Send to Jev'}<Arrow /></button><p className="public-note" id="public-note"><span aria-hidden="true">↗</span> Accepted notes are public. Leave out personal details.</p>{sendError && <p className="inline-error" role="alert">{sendError}</p>}
            </form>
          </section>
          {receipts[0] ? <Receipt receipt={receipts[0]} openBin={openBin} /> : <div className="first-note"><span className="doodle-arrow">↳</span><p>A real little moment.<br /><span>Everyone here sees the same Jev.</span></p></div>}
    ...
    <footer className="site-footer"><span>Made for the things on your mind.</span><span>{total} notes filed with care <span className="footer-flower">✳</span></span></footer>
