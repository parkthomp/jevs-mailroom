...
A shared pixel-art mailroom that fills the screen and that you walk around in. Jev is the mailroom’s little robot, and every visitor gets a little character of their own: walk with the arrow keys or WASD (or the on-screen pad and A button on touch screens, or by clicking where to go). Walk up to the **INCOMING** desk to leave Jev a short message, watch him carry the envelope to **Compliments, Ideas, Complaints, or Misc**, then walk up to a bin to read its messages. New visitors pick a name before walking in, and it floats over their character as a name tag. Everyone in the room sees everyone else's character and name. The **Menu** button offers the same actions without walking. Screened-out messages get a visible toss into the trash; their contents stay private. There are no correction or recategorization controls.

React and Canvas draw the room. An Express/WebSocket server shares its state (character positions and name tags are relayed over the same socket and kept only in the web server's memory; names are limited to 12 capitals, digits, and simple punctuation and are not screened), and a worker asks [Jev](https://openrouter.ai/typesafe/jev-1.13), TypeSafe’s decision model, through OpenRouter to screen messages, pick their bin, and pick Jev’s reaction. The server controls delivery timing so every visitor watches the same Jev.
