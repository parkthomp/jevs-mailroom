...
A shared pixel-art mailroom that fills the screen and that you walk around in. Jev is the mailroom’s little robot, and every visitor gets a little character of their own: walk with the arrow keys or WASD (or the on-screen pad and A button on touch screens, or by clicking where to go). Walk up to the **INCOMING** desk to leave Jev a short message, watch him carry the envelope to **Compliments, Feedback, Important, Big Ideas, Dad Jokes, Art, or Spam**, then walk up to a bin to read its messages. Walk up to Jev while he's milling about and press Space (or click him) to say hi: he stops, turns to you, and says one of a few canned lines in a speech bubble for five seconds that everyone in the room sees, then wanders on. He doesn't chat while there's mail to sort. New visitors pick a name before walking in; Jev checks it, and it floats over their character as a name tag and signs the notes they send. Everyone in the room sees everyone else's character and name. The **Menu** button offers the same actions without walking. Screened-out messages get a visible toss into the trash; their contents stay private. There are no correction or recategorization controls.

React and Canvas draw the room. An Express/WebSocket server shares its state (character positions and name tags are relayed over the same socket and kept only in the web server's memory; names are limited to 12 capitals, digits, and simple punctuation, and Jev turns away obscene or attacking ones before they're shown; an approved name gets a server-signed pass the socket and mail tray require, and it signs that visitor's published notes), and a worker asks [Jev](https://openrouter.ai/typesafe/jev-1.13), TypeSafe’s decision model, through OpenRouter to screen messages, pick their bin, and pick Jev’s reaction. The server controls delivery timing so every visitor watches the same Jev.
...
Accepted messages are public and remain browsable in their bin. The sender’s receipt token reveals private status; it is not part of public room events. Screening uses a PG-13 bar: hate or severe targeted harassment, exposed private information or credentials, explicit sexual content, graphic violence, and real-world threats or encouragement of harm go to trash. Jev can also choose trash directly for anything else unfit to publish, and that choice wins over every bin, including Important. Mild profanity, ordinary criticism, playful jokes, and fictional action are publishable. Harmless advertising, gibberish, and test notes go to Spam. The server privately routes all Star Wars references to Important, ahead of other categories; public labels and reactions do not explain this rule. The public trash animation uses fixed text and never exposes the original message or screening explanation.

On the first start of the updated coordinator, a durable, one-time migration queues **every saved message**, including previously discarded mail, for fresh screening and categorization by Jev. The archive is temporarily hidden while messages are checked and re-delivered. IDs, sender names, receipt tokens, text, and original timestamps are preserved. Re-sorting uses the normal daily budget, retries, and restart recovery; large archives may continue the next day. Deploy both web and worker versions. The re-sort starts only once the new worker acquires leadership, so the pre-deploy migration does not queue it for the old worker.

A second one-time pass follows the addition of trash as a bin choice: every message filed in a bin (or still in flight) is hidden and checked again so anything that should have been thrown away goes to the trash. Messages already discarded by the previous re-sort stay discarded and are not re-checked. A room that never ran the first re-sort gets a single full pass under the new rules.
