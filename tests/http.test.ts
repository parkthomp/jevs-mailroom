...
test('HTTP + WebSockets share a durable room and never disclose a discarded message', { timeout: 30000 }, async () => {
  ...
  const env = { ...process.env, PORT: String(port), DATA_FILE: join(directory, 'state.json'), AI_MODE: 'demo', NODE_ENV: 'test', DATABASE_URL: '', REDIS_URL: '', RECEIPT_SECRET: 'integration-test-secret-that-is-not-used-elsewhere' };
  let output = '';
  const start = () => {
