...
import { FACINGS, LOOKS, type Facing, type Visitor } from '../shared/protocol.js';
...
// by a visitor, so there is nothing to screen.
...
export class Presence<Socket extends object> {
  ...
  move(socket: Socket, event: unknown, now = Date.now()): boolean {
    ...
    const { type, x, y, facing, look, moving } = event as Record<string, unknown>;
    ...
    entry.visitor = { id: entry.id, x: clamp(x, ROOM_W), y: clamp(y, ROOM_H), facing: facing as Facing, look: look as number, moving };
    this.dirty = true;
    return true;
  }
  list(): Visitor[] {
